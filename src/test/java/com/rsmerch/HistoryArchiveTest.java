...
public class HistoryArchiveTest {
    ...
    @Test public void reimportAndReloadAreIdempotentButIdenticalOccurrencesArePreserved() throws Exception {
        HistoryArchive archive=new HistoryArchive(temp.getRoot().toPath(),ACCOUNT);
        ...
        HistoryArchive reloaded=new HistoryArchive(temp.getRoot().toPath(),ACCOUNT);
    ...
    @Test public void screenSnapshotRepeatsDoNotMultiplyButDifferentSnapshotsRemainReviewable() throws Exception {
        ...
        HistoryArchive.View v=new HistoryArchive(temp.getRoot().toPath(),ACCOUNT).view(new Book());
    ...
    @Test public void wrongAccountOrEditedFinancialFieldsFailClosed() throws Exception {
        Path root=temp.getRoot().toPath(); HistoryArchive archive=new HistoryArchive(root,ACCOUNT);
        archive.save(HistoryArchive.RUNELITE,parse(saved()),saved(),NOW);
        try { new HistoryArchive(root,"222222222222222222222222"); fail(); } catch (java.io.IOException expected) { }
        ...
        try { new HistoryArchive(root,ACCOUNT); fail(); } catch (java.io.IOException expected) { }
    }
